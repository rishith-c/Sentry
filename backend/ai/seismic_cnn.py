...
_AMD_GPU_AVAILABLE = os.getenv("AMD_GPU_AVAILABLE", "").lower() in ("true", "1")
if _TORCH_AVAILABLE:
    DEVICE = torch.device("cuda" if _AMD_GPU_AVAILABLE else "cpu")
    logger.info("CNN device: %s", DEVICE)
else:
    DEVICE = None
    logger.warning("torch not installed — seismic CNN inference disabled")

_WEIGHTS_PATH = Path(__file__).parent / "sentinel_cnn.pth"
_INPUT_CHANNELS = 3
_INPUT_LENGTH = 500
_POOL_SIZE = 50
...
_model: "SentryCNN | None" = None


class SentryCNN(nn.Module):
...
def run_inference(waveform: np.ndarray) -> float:
    ...
    logger.info("[SENTRY] AMD MI300X inference: %.1fms", elapsed_ms)
    print(f"[SENTRY] AMD MI300X inference: {elapsed_ms:.1f}ms")  # noqa: T201
