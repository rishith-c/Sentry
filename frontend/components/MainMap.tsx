...
export default function MainMap() {
  ...
  useEffect(() => {
    ...
    const crewMarkers = crewMarkersRef.current;
    ...
    // WS status HUD
    const wsHud = new L.Control({ position: 'bottomleft' });
    wsHud.onAdd = () => {
  ...
  useEffect(() => {
    ...
    if (!layer) return;
    layer.clearLayers();
    if (!layers.fire || storeHotspots.length === 0) return;
  ...
  }, [layers.seismic, storeDamageCells]);
  ...
  useEffect(() => {
    ...
    if (!layer) return;
    layer.clearLayers();
    if (!layers.ember || storeEmberZones.length < 4) return;
  ...
  }, [layers.crews, storeCrews]);

  useEffect(() => {
    const layer = dynEpicenterRef.current;
    if (!layer) return;
    layer.clearLayers();
    if (!layers.seismic || storeSeismicEvents.length === 0) return;
  ...
  }, [layers.seismic, storeSeismicEvents]);

  useEffect(() => {
    const map = mapRef.current;
    const infraLayer = infraLayerRef.current;
    if (!map || !infraLayer) return;

    if (layers.infrastructure) {
      if (!map.hasLayer(infraLayer)) {
